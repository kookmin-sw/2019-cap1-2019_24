...
def returnFaceInfo(request):
    ...
    for i in file_list:
        face_recog(i, path_dir)
        os.remove(path_dir + i)

    return JsonResponse(faceInfoList)
...
def face_recog(imageFileName, path_dir):
    f_imageFileName = path_dir + imageFileName

    global faceInfoList

    #각 이미지마다 face detect 및 box 생성
    image = get_image(f_imageFileName)
    ...
    #이미지에 detect된 얼굴이 있으면 vector값을 받아서 db에 저장
        ...
        for n in range(0, len(face_boxes)):
            ...
            faceInfo = {
                ...
                'bottom' : bottom,
                'left' : left,
                'vector' : str(face_vector_list[n]),
            }
            faceInfoList["faceInfos"].append(faceInfo)
