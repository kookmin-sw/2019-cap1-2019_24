...
//            }
    ...
    void deleteNetworkEdge(SQLiteDatabase db, String imgno, String delete_tag) {
        ...
        for (int i = 0; i < tmp_tag_list.size(); i++) {
            ...
            if (r1 == 1 && r2 == 0) {
                ...
                update_weight = cursor.getInt(0) - 1;
                if (update_weight == 0) {
                    db.execSQL("DELETE FROM Tag_network WHERE tag_1_id = '" + v + "' AND tag_2_id = '" + u + "'");
                } else {
                    db.execSQL("UPDATE Tag_network SET weight = " + update_weight + "  WHERE tag_1_id = '" + v + "' AND tag_2_id = '" + u + "'");
                }
            }
    ...
    void insertAutoTag(String ImageFileName, ArrayList<String> taglist) {
        SQLiteDatabase db = getWritableDatabase();
        String sql = "SELECT * FROM Image_info WHERE imageFileName = '" + ImageFileName + "'";
        Cursor cursor2;
        cursor2 = db.rawQuery(sql, null);
        if (cursor2.getCount() != 0) {
            cursor2.moveToFirst();
            int imgno = cursor2.getInt(1);
            String imgfilepath = cursor2.getString(3);
            ...
            for (String item : taglist) {
                ContentValues values = new ContentValues();
                ContentValues values2 = new ContentValues();
                String eng_name = item;

                //Auto_Tag에서 정보 가져오기
                sql = "SELECT tagID, name_kor, tagState FROM Auto_Tag WHERE name_eng = '" + eng_name + "'";
                Cursor cursor;
                cursor = db.rawQuery(sql, null);
                //받아와야하는 것 : tagID, name_kor, tagState
                cursor.moveToFirst();
                //tag id
                tagID = cursor.getString(0);
                //name_kor
                String name_kor = cursor.getString(1);
                //cursor는 boolean을 못 받아온다고 해서 방법 못찾으면 integer로 바꿔야할 것 같아요.
                //우선 integer로 해서 체크 했어용
                int bool = cursor.getInt(2);
                ...
                if (bool != 0) {
                    ...
                    //ImageNo이랑 imgFilePath를 받아와서 같이 넣어야하는데
                    //ImageNo
                    values.put("ImageNo", imgno);
                    values.put("imgFilePath", imgfilepath);

                    values.put("tagID", tagID);
                    values.put("tag", name_kor);

                    db.insert("Tag_Log", null, values);

                    //tagState가 0일때만 tmp_id에 넣어주기
                    tmp_id.add(tagID);

                    //Total_Tag_Info에 넣기
                    sql = "SELECT tag_freq FROM Total_Tag_Info WHERE tag_id = '" + tagID + "'";
                    cursor = db.rawQuery(sql, null);
                    int tmp_c = cursor.getCount();
                    int update_freq;

                    if (tmp_c == 1) {
                        cursor.moveToFirst();
                        update_freq = cursor.getInt(0) + 1;
                        db.execSQL("UPDATE Total_Tag_Info SET tag_freq = " + update_freq + " WHERE tag_id = '" + tagID + "';");
                    } else if (tmp_c == 0) {
                        values2.put("tag_id", tagID);
                        values2.put("tag_name", name_kor);
                        values2.put("tag_freq", 1);
                        db.insert("Total_Tag_Info", null, values2);
                    }
                }

            }

            createNetworkEdge(tmp_id);
        }
        db.close();
    }
    ...
    public ArrayList<String> imgpath(){
        ...
        String sql = "SELECT imgFilePath FROM Image_info";
        Cursor cursor;
        cursor = db.rawQuery(sql,null);
        while(cursor.moveToNext()){
    ...
    void insertFaceInfo(String imageFileName,int top, int right, int left, int bottom, String vector){
        SQLiteDatabase db = getWritableDatabase();
        String sql = "SELECT * FROM Image_info WHERE imageFileName = '" + imageFileName + "'";
        Cursor cursor2;
        cursor2 = db.rawQuery(sql, null);
        if (cursor2.getCount() != 0) {
            ContentValues values2 = new ContentValues();
            values2.put("imgFileName", imageFileName);
            values2.put("imgLeft", left);
            values2.put("imgTop", top);
            values2.put("imgRight", right);
            values2.put("imgBottom", bottom);
            values2.put("vectorValue", vector);
            db.insert("Face_Vector_Value", null, values2);
        }
        db.close();
