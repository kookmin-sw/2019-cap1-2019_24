...
def face_recog(database_file, path_dir, new_image_file_list):
    #데이터베이스 열기
    ...
    for i in image_file_name:
        ...
        if(len(face_boxes) == 0):
            pass
        #이미지에 detect된 얼굴이 있으면 vector값을 받아서 db에 저
        else:
            face_vector_lists = get_face_vector(image, face_boxes)
            list(face_boxes)
            list(face_vector_lists)
            for n in range(0, len(face_boxes)):
                ...
                cur.execute(sql, (i,top, right, bottom, left, face_vector_lists[n]))
                con.commit()
    con.close()
